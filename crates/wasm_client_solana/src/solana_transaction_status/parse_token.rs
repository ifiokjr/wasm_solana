...
use extension::default_account_state::*;
use extension::group_member_pointer::*;
use extension::group_pointer::*;
use extension::interest_bearing_mint::*;
...
use serde_json::json;
use solana_program::instruction::Instruction as SplTokenInstruction;
...
use crate::solana_account_decoder::parse_account_data::SplTokenAdditionalDataV2;
use crate::solana_account_decoder::parse_token::UiAccountState;
use crate::solana_account_decoder::parse_token::token_amount_to_ui_amount_v3;
		...
		match token_instruction {
			...
			TokenInstruction::InitializeMultisig { m } => {
				...
				for index in &instruction.accounts[2..] {
					signers.push(account_keys[*index as usize].to_string());
				}
			...
			TokenInstruction::InitializeMultisig2 { m } => {
				...
				for index in &instruction.accounts[1..] {
					signers.push(account_keys[*index as usize].to_string());
				}
...
fn parse_signers(
	...
	if accounts.len() > last_nonsigner_index + 1 {
		let mut signers: Vec<String> = vec![];
		for index in &accounts[last_nonsigner_index + 1..] {
			signers.push(account_keys[*index as usize].to_string());
		}
		map.insert(
