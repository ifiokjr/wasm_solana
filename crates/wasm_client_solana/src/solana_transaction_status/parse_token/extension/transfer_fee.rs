...
pub(in crate::solana_transaction_status::parse_token) fn parse_transfer_fee_instruction(
	instruction_data: &[u8],
	account_indexes: &[u8],
	...
	match transfer_fee_instruction {
		...
		TransferFeeInstruction::WithdrawWithheldTokensFromAccounts { num_token_accounts } => {
			...
			for ii in &account_indexes[first_source_account_index..] {
				source_accounts.push(account_keys[*ii as usize].to_string());
			}
