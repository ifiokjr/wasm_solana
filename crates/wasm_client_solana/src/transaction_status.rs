...
// spl_associated_token_id();     static ref BPF_LOADER_PROGRAM_ID: Pubkey =
// solana_sdk::bpf_loader::id();     static ref
// BPF_UPGRADEABLE_LOADER_PROGRAM_ID: Pubkey =
// solana_sdk::bpf_loader_upgradeable::id();     static ref MEMO_V1_PROGRAM_ID:
// Pubkey = spl_memo_id_v1();     static ref MEMO_V3_PROGRAM_ID: Pubkey =
// spl_memo_id_v3();     static ref STAKE_PROGRAM_ID: Pubkey =
// stake::program::id();     static ref SYSTEM_PROGRAM_ID: Pubkey =
// system_program::id();     static ref VOTE_PROGRAM_ID: Pubkey =
// solana_vote_program::id();     static ref PARSABLE_PROGRAM_IDS:
// HashMap<Pubkey, ParsableProgram> = {         let mut m = HashMap::new();
//         m.insert(
...
//         ParsableProgram::SplAssociatedTokenAccount => {
//             serde_json::to_value(parse_associated_token(instruction,
// account_keys)?)?         }
//         ParsableProgram::SplMemo => parse_memo(instruction)?,
//         ParsableProgram::SplToken =>
// serde_json::to_value(parse_token(instruction, account_keys)?)?,
//         ParsableProgram::BpfLoader => {
//             serde_json::to_value(parse_bpf_loader(instruction,
// account_keys)?)?         }
//         ParsableProgram::BpfUpgradeableLoader => {
//             serde_json::to_value(parse_bpf_upgradeable_loader(instruction,
// account_keys)?)?         }
...
//         ParsableProgram::Vote => serde_json::to_value(parse_vote(instruction,
// account_keys)?)?,     };
	...
	fn encode_with_meta(
		...
		match encoding {
			...
			UiTransactionEncoding::Base64 => {
				EncodedTransaction::Binary(
					STANDARD.encode(bincode::serialize(self).unwrap()),
