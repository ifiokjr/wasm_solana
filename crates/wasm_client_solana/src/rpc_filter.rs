...
// Internal struct to hold Memcmp filter data as either encoded String or raw
// Bytes
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(untagged)]
enum DataType {
...
// Internal struct to enable Memcmp filters with explicit Base58 and Base64
// encoding. The From implementations emulate `#[serde(tag = "encoding", content
// = "bytes")]` for `MemcmpEncodedBytes`. On the next major version, all these
// internal elements should be removed and replaced with adjacent tagging of
// `MemcmpEncodedBytes`.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
struct RpcMemcmp {
	offset: usize,
...
impl From<RpcMemcmp> for Memcmp {
	fn from(memcmp: RpcMemcmp) -> Memcmp {
		...
		let bytes = match (encoding, memcmp.bytes) {
			(RpcMemcmpEncoding::Binary | RpcMemcmpEncoding::Base58, DataType::Encoded(string)) => {
				MemcmpEncodedBytes::Base58(string)
			}
